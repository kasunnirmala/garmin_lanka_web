...
extract( // @codingStandardsIgnoreLine
	shortcode_atts(
		array(
			...
			'icon_col'      => '#333',
			'icon_bg_col'   => '',
			'heading'       => 'This is Riode Icon Box',
			'head_size'     => 16,
			'head_weight'   => 700,
			'head_mt'       => 0,
			'head_mr'       => 0,
...
echo '<div id="riode_gtnbg_icon_box_' . $id . '" class="' . esc_attr( $className ) . '">';
echo '<style>';

echo '#riode_gtnbg_icon_box_' . $id . ' .icon-box {';
foreach ( $box_css as $key => $value ) {
	echo esc_attr( $key . ':' . $value . ';' );
}
echo '}';

echo '#riode_gtnbg_icon_box_' . $id . ' .icon-box-content {';
foreach ( $content_css as $key => $value ) {
	echo esc_attr( $key . ':' . $value . ';' );
}
echo '}';

echo '#riode_gtnbg_icon_box_' . $id . ' figure{';
foreach ( $figure_css as $key => $value ) {
	echo esc_attr( $key . ':' . $value . ';' );
}
echo '}';

echo '#riode_gtnbg_icon_box_' . $id . ' .icon-box i{';
foreach ( $icon_css as $key => $value ) {
	echo esc_attr( $key . ':' . $value . ';' );
}
echo '}';

echo '#riode_gtnbg_icon_box_' . $id . ' .icon-box .icon-box-title{';
foreach ( $head_css as $key => $value ) {
	echo esc_attr( $key . ':' . $value . ';' );
}
echo '}';

echo '#riode_gtnbg_icon_box_' . $id . ' .icon-box p{';
foreach ( $desc_css as $key => $value ) {
	echo esc_attr( $key . ':' . $value . ';' );
}
echo '}';

echo '#riode_gtnbg_icon_box_' . $id . ' .image-body{';
echo 'flex: none;';
echo '}';
echo '</style>';
...
if ( 'mixed' !== $type ) {
	echo '<h4 class="icon-box-title">' . riode_strip_script_tags( $heading ) . '</h4>';
} else {
	echo '<h4 class="icon-box-title">';
	echo '<span class="icon-box-icon">';
	echo '<i class="' . esc_attr( $icon_class ) . '"></i>';
	echo '</span>';
	echo riode_strip_script_tags( $heading );
	echo '</h4>';
}

echo '<p class="description">' . riode_strip_script_tags( $description ) . '</p>';
echo '</div>';
echo '</div>';
echo '</div>';
