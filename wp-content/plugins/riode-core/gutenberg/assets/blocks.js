...
/**
 * Register Gutenberg Riode Block
 */
(function (wpI18n, wpBlocks, wpElement, wpBlockEditor, wpComponents, wpData, lodash, apiFetch) {
	...
	// Register Riode Slider Block
	registerBlockType('riode/riode-slider', {
		title: 'Riode Slider',
		icon: 'riode',
		category: 'riode',
		attributes: {
			...
			animation_in: { type: 'string', default: '', },
			animation_out: { type: 'string', default: '', },
			nav_size: { type: 'int', default: 20, },
		},
		supports: {
			align: ['wide', 'full'],
		},
		edit: riodeComponents.RiodeSlider,
		save: function (props) {
			return el(InnerBlocks.Content);
		}
	});

	// Register Riode Banner Block
	registerBlockType('riode/riode-banner', {
		title: 'Riode Banner',
		icon: 'riode',
		category: 'riode',
		attributes: {
		...
		edit: riodeComponents.RiodeBanner,
		save: function (props) {
			return el(InnerBlocks.Content);
		}
	});

	// Register Riode Products Block
	registerBlockType('riode/riode-products', {
		title: 'Riode Products',
		icon: 'riode',
		category: 'riode',
		attributes: {
			...
			hover_change: { type: 'boolean', default: true, },
			content_align: { type: 'string', default: 'left', },
			addtocart_pos: { type: 'string', default: '', },
			quickview_pos: { type: 'string', default: 'bottom', },
			wishlist_pos: { type: 'string', default: '', },
		},
		edit: riodeComponents.RiodeProducts,
		save: function (props) {
			return el(InnerBlocks.Content);
		}
	});

	// Register Riode Categories Block
	registerBlockType('riode/riode-categories', {
		title: 'Riode Categories',
		icon: 'riode',
		category: 'riode',
		attributes: {
		...
		edit: riodeComponents.RiodeCategories,
		save: function (props) {
			return el(InnerBlocks.Content);
		}
	});

	// Register Riode Posts Block
	registerBlockType('riode/riode-posts', {
		title: 'Riode Posts',
		icon: 'riode',
		category: 'riode',
		attributes: {
			...
			excerpt_limit: { type: 'int', default: 20 },
		...
		edit: riodeComponents.RiodePosts,
		save: function (props) {
			return el(InnerBlocks.Content);
		}
	});

	// Register Riode Icon-Box Block
	registerBlockType('riode/riode-icon-box', {
		title: 'Riode Icon Box',
		icon: 'riode',
		category: 'riode',
		attributes: {
			...
			heading: { type: 'string', default: 'This is Riode Icon Box' },
		...
		edit: riodeComponents.RiodeIconBox,
		save: function (props) {
			return el(InnerBlocks.Content);
		}
	});

	// Register Riode Heading Block
	registerBlockType('riode/riode-heading', {
		title: 'Riode Heading',
		icon: 'riode',
		category: 'riode',
		attributes: {
			content_align: { type: 'string', default: 'left' },
			text: { type: 'string', default: 'This is Riode Heading' },
		...
		edit: riodeComponents.RiodeHeading,
		save: function (props) {
			return el(InnerBlocks.Content);
		}
	});

	// Register Riode Button Block
	registerBlockType('riode/riode-button', {
		title: 'Riode Button',
		icon: 'riode',
		category: 'riode',
		attributes: {
		...
		edit: riodeComponents.RiodeButton,
		save: function (props) {
			return el(InnerBlocks.Content);
		}
	});
...
// Defines Riode Component Classes
function componentClassInit(wpI18n, wpElement, wpBlockEditor, wpComponents, apiFetch) {
	...
	var sliderStyleOptions = function (attrs, props) {
		return el(PanelBody, {
			...
			el(SelectControl, {
				label: __('Nav Type'),
				options: [{ label: __('Default'), value: '' }, { label: __('Simple'), value: 'simple' }, { label: __('Circle'), value: 'circle' }, { label: __('Full'), value: 'full' }],
			...
			el(TextControl, {
				...
				value: attrs.animation_in,
				onChange: (value) => { props.setAttributes({ animation_in: value }); },
			}),
			el(TextControl, {
				label: __('Animation Out'),
				value: attrs.animation_out,
				onChange: (value) => { props.setAttributes({ animation_out: value }); },
			}),
		);
	}

	riodeComponents.RiodeSlider = class RiodeSlider extends Component {
		constructor() {
			super(...arguments);
		}
		...
		componentDidMount() {
			var attrs = this.props.attributes,
				$selector = jQuery('#block-' + this.props.clientId + ' .owl-carousel>div>.block-editor-block-list__layout');
		...
		render() {
			...
			extra_class += 'owl-carousel owl-theme cols-' + Number(attrs.col_cnt);
			...
			var renderControls = el(
				'div',
				{ className: extra_class, 'data-plugin': 'owl', 'data-plugin-options': extra_attr },
				el(InnerBlocks),
			);
	...
	riodeComponents.RiodeBanner = class RiodeBanner extends Component {
		constructor() {
			super(...arguments);
		}
	...
	riodeComponents.RiodeProducts = class RiodeProducts extends Component {
		...
		render() {
			...
			var inspectorControls = el(InspectorControls, {},
				...
				el(PanelBody, {
					...
					'grid' === attrs.layout_type && el(ToggleControl, {
						label: __('Split Line'),
						checked: attrs.split_line,
						onChange: (value) => { props.setAttributes({ split_line: value }); },
					}),
					el(SelectControl, {
						label: __('Item count'),
						value: attrs.col_cnt,
				...
				el(PanelBody, {
					...
					el(ToggleControl, {
						label: __('Change Image on Hover'),
						checked: attrs.hover_change,
						onChange: (value) => { props.setAttributes({ hover_change: value }); },
					}),
					el(SelectControl, {
					...
					'' === attrs.product_type && el(SelectControl, {
						label: __('Add to Cart Pos'),
						options: [{ value: '', label: __('On top of the image') }, { value: 'bottom', label: __('On bottom of the image') }, { value: 'hide', label: __('Don\'t show') }],
						value: attrs.addtocart_pos,
						onChange: (value) => { props.setAttributes({ addtocart_pos: value }); },
					}),
					'' === attrs.product_type && el(SelectControl, {
						label: __('Quickview Pos'),
						options: [{ value: '', label: __('On top of the image') }, { value: 'bottom', label: __('On bottom of the image') }, { value: 'hide', label: __('Don\'t show') }],
						value: attrs.quickview_pos,
						onChange: (value) => { props.setAttributes({ quickview_pos: value }); },
					}),
					'' === attrs.product_type && el(SelectControl, {
						label: __('Add to wishlist Pos'),
						options: [{ value: '', label: __('On top of the image') }, { value: 'bottom', label: __('On bottom of the image') }, { value: 'hide', label: __('Don\'t show') }],
						value: attrs.wishlist_pos,
						onChange: (value) => { props.setAttributes({ wishlist_pos: value }); },
					}),
				),
			);
		...
		getQuery() {
			...
			var endpoint = '/rioderest/wcrest/products' + query_string;
			return endpoint;
		}
				...
				el(
					...
					el(
						...
						el(
							...
							el(
								'img',
								{ src: product.images.length ? product.images[0].src : riode_gutenberg_vars.placeholder_img },
							),
							2 <= product.images.length && attrs.hover_change && el(
								'img',
	...
	riodeComponents.RiodeCategories = class RiodeCategories extends Component {
		...
		componentDidMount() {
			...
			if ('creative' === this.props.attributes.layout_type) {
				this.creative_layout = riode_creative_layout(this.props.attributes.creative_mode);
				...
				el(PanelBody, {
					...
					el(SelectControl, {
						...
						onChange: (value) => {
							...
							if ('creative' === value) {
								this.creative_layout = riode_creative_layout(attrs.creative_layout);
					...
					'creative' === attrs.layout_type && el(SelectControl, {
						...
						onChange: (value) => {
							...
							this.creative_layout = riode_creative_layout(Number(value));
							props.setAttributes({ count: this.creative_layout.length });
						},
					}),
		...
		createCategoryElement(cat, idx) {
			...
			// Category Type
			if ('badge' === attrs.category_type) {
				cat_class.push('cat-type-badge cat-type-absolute');
				...
				el(
					...
					'label' !== attrs.category_type && el(
						...
						el(
							...
							!attrs.show_icon && el(
								'img',
								{ src: cat.image && cat.image.src ? cat.image.src : riode_gutenberg_vars.placeholder_img },
							),
					...
					'label' !== attrs.category_type && el(
						...
						('classic' === attrs.category_type || 'badge' === attrs.category_type || 'banner' === attrs.category_type || 'group' === attrs.category_type) && el(
							'h3',
							{ className: 'woocommerce-loop-category__title' },
							cat.name,
							'badge' === attrs.category_type && attrs.show_count && el(
								'mark',
								{},
								'(' + cat.count + ')',
							),
						...
						attrs.show_link && ('classic' === attrs.category_type || 'badge' === attrs.category_type || 'banner' === attrs.category_type || 'icon' === attrs.category_type) && el(
							'a',
							{ className: 'badge' === attrs.category_type ? 'btn btn-primary btn-block' : 'btn btn-underline btn-link', href: "#", },
							attrs.link_text,
						)
					),
				),
	...
	riodeComponents.RiodePosts = class RiodePosts extends Component {
		constructor() {
			super(...arguments);
			...
			var inspectorControls = el(InspectorControls, {},
				...
				el(PanelBody, {
					...
					attrs.excerpt_custom && el(RangeControl, {
						...
						value: attrs.excerpt_limit,
						min: 1,
						max: 500,
						step: 1,
						onChange: (value) => { props.setAttributes({ excerpt_limit: value }); },
					}),
				),
				el(PanelBody, {
					...
					el(SelectControl, {
						...
						options: [{ label: __('Primary'), value: 'btn-primary' }, { label: __('Secondary'), value: 'btn-secondary' }, { label: __('Alert'), value: 'btn-alert' }, { label: __('Success'), value: 'btn-success' }, { label: __('Dark'), value: 'btn-dark' }, { label: __('White'), value: 'btn-white' }],
	...
	riodeComponents.RiodeIconBox = class RiodeIconBox extends Component {
		constructor() {
			super(...arguments);
		}
	...
	riodeComponents.RiodeHeading = class RiodeHeading extends Component {
		constructor() {
			super(...arguments);
		}
		...
		render() {
			...
			var inspectorControls = el(InspectorControls, {},
				...
				el(PanelBody, {
					...
					'title-underline' === attrs.decoration && el(ToggleControl, {
						label: __('Hide Active Underline?'),
						checked: attrs.hide_active_underline,
						onChange: (checked) => { props.setAttributes({ hide_active_underline: checked }); },
					}),
		...
		createHeadingElement() {
			...
			} else if ('title-underline' === attrs.decoration) {
	...
	riodeComponents.RiodeButton = class RiodeButton extends Component {
		constructor() {
			super(...arguments);
		}
...
function isSliderChanged(attrs, prevAttrs) {
	if (attrs.show_nav !== prevAttrs.show_nav
		...
		|| attrs.animation_in !== prevAttrs.animation_in
		|| attrs.animation_out !== prevAttrs.animation_out) {
		return true;
	}
	return false;
}
...
function getSliderClass(attrs) {
	var extra_class = ' owl-carousel owl-theme';
	if ('full' === attrs.nav_type) {
		extra_class += ' owl-nav-full';
	} else {
		if ('simple' === attrs.nav_type) {
			extra_class += ' owl-nav-simple';
		}
		if ('inner' === attrs.nav_pos) {
			extra_class += ' owl-nav-inner';
		} else if ('top' === attrs.nav_pos) {
	...
	if (attrs.dots_type) {
		extra_class += ' owl-dot-' + attrs.dots_type;
	}

	if ('inner' === attrs.dots_pos) {
		extra_class += ' owl-dot-inner';
	}
	if ('close' === attrs.dots_pos) {
	...
	if ('top' === attrs.slider_vertical_align ||
		...
		extra_class += ' owl-' + attrs.slider_vertical_align;
	}
	return extra_class;
}
...
function getSliderOptions(attrs) {
	var extra_options = {},
		breakpoints = riode_gutenberg_vars.breakpoints;
	...
	extra_options["animateIn"] = attrs.animation_in;
	extra_options["animateOut"] = attrs.animation_out;
	extra_options["items"] = Math.max(Number(attrs.col_cnt), 1);
 ...
 * @since 1.0
 */
function initOwlCarousel($selector, $option = {}) {
	...
	if ($selector.length) {
		...
		var newOwlSettings = jQuery.extend(true, {}, sliderDefaultOptions, pluginOptions);
		newOwlSettings = jQuery.extend(true, {}, newOwlSettings, $option)

		$selector.on('initialize.owl.carousel', function (e) {
			...
			this.classList.remove('gutter-xs');
			this.classList.remove('gutter-sm');
			this.classList.remove('gutter-md');
			this.classList.remove('gutter-lg');
		});

		$selector.imagesLoaded(function () {
			var owlIns = $selector.owlCarousel(newOwlSettings);
		});
	}
}
...
function initIsotopes($selector) {
	...
	$selector.find('.owl-carousel').on('resized.owl.carousel', function (e) {
