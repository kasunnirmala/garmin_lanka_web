 ...
 * @since 1.1.0
 */
jQuery('.riode-wpb-number-container .riode-responsive-toggle').on('click', function (e) {
    var $this = jQuery(this);
    $this.parent().toggleClass('show');
});

if (undefined == riode_core_vars.wpb_editor || undefined == riode_core_vars.wpb_editor.riode_number_included || true != riode_core_vars.wpb_editor.riode_number_included) {
    jQuery(document.body).on('click', '.riode-wpb-number-container .riode-responsive-span li', function (e) {
        var $this = jQuery(this),
            $dropdown = $this.closest('.riode-responsive-dropdown'),
            $toggle = $dropdown.find('.riode-responsive-toggle'),
            $control = $dropdown.parent(),
            $input = $control.find('.riode-wpb-number');
        // Actions
    ...
    }).off('responsive_changed', '.riode-wpb-number-container .riode-responsive-span li').on('responsive_changed', '.riode-wpb-number-container .riode-responsive-span li', function (e) {
        console.log('responsive_changed');
        var $this = jQuery(this),
            $dropdown = $this.closest('.riode-responsive-dropdown'),
            $toggle = $dropdown.find('.riode-responsive-toggle'),
            $control = $dropdown.parent(),
        ...
        $control.data('width', width);
        $input.val($input.data(width) ? $input.data(width) : '');
    }).on('change', '.riode-wpb-number', function (e) {
        var $this = jQuery(this),
            $control = $this.parent(),
            $form = $control.next();
    ...
    }).on('change', '.riode-wpb-units', function (e) {
        var $this = jQuery(this),
            $control = $this.parent(),
            $form = $control.next(),
            $input = $control.find('.riode-wpb-number');
    ...
    if (undefined == riode_core_vars.wpb_editor) {
        riode_core_vars.wpb_editor = {
            riode_number_included: true,
        }
    } else {
        riode_core_vars.wpb_editor.riode_number_included = true;
    }
}