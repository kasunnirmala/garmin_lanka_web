...
// Preprocess
$wrapper_attrs = array(
	'class' => 'riode-masonry-item-container ' . $shortcode_class . $style_class,
);
...
if ( ! isset( $atts['creative_height'] ) ) {
	$atts['creative_height'] = json_encode(
		array(
			'xl' => 'preset',
		)
	);
}
if ( ! isset( $atts['creative_order'] ) ) {
	$atts['creative_order'] = json_encode(
		array(
			'xl' => '',
		)
	);
...
$settings = array_merge( array(), riode_wpb_convert_responsive_values( 'creative_width', $atts, 0 ) );
$settings = array_merge( $settings, riode_wpb_convert_responsive_values( 'creative_height', $atts, 'preset' ) );
$settings = array_merge( $settings, riode_wpb_convert_responsive_values( 'creative_order', $atts, '' ) );

$grid = array();

global $riode_wpb_creative_layout;
if ( isset( $riode_wpb_creative_layout ) && ! vc_is_inline() ) {
	$idx = $riode_wpb_creative_layout['index'];
	if ( $idx < count( $riode_wpb_creative_layout['preset'] ) ) {
		foreach ( $riode_wpb_creative_layout['preset'][ $idx ] as $key => $value ) {
			if ( 'h' == $key ) {
				continue;
			}
...
if ( 'preset' == $settings['creative_height_xl'] ) {
	if ( ! vc_is_inline() ) {
		$grid['h'] = $idx < count( $riode_wpb_creative_layout['preset'] ) ? $riode_wpb_creative_layout['preset'][ $idx ]['h'] : '1-3';
	}
...
if ( isset( $riode_wpb_creative_layout ) && ! vc_is_inline() ) {
	$riode_wpb_creative_layout['layout'][ $idx ] = $grid;
	$riode_wpb_creative_layout['index']          = ++$idx;
}
