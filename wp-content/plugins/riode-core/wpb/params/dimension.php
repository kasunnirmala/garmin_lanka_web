<?php
/**
 * Riode WPBakery Dimension Callback
 *
 * adds dimension control for element option
 * follow below example of riode_dimension control
 *
 * array(
 *      'type'        => 'riode_dimension',
 *      'heading'     => __( 'Buttton Padding', 'riode-core' ),
 ...
 * @return string
 */
function riode_dimension_callback( $settings, $value ) {
	...
	$disallowed    = isset( $settings['disallowed'] ) ? $settings['disallowed'] : array();
	$class         = 'riode-wpb-dimension-container';
	...
	foreach ( $dimensions as $dimension => $label ) {
		...
		<input type="text"
			...
			<?php echo in_array( $dimension, $disallowed ) ? ' disabled' : ''; ?>
			/>
...
vc_add_shortcode_param( 'riode_dimension', 'riode_dimension_callback', plugins_url( '../js/riode-dimension.js', __FILE__ ) );
