<?php
/**
 * Riode Responsive Callback
 *
 * adds responsive control for element option
 * follow below example of riode_responsive control
 *
 *
 * @since 1.1.0
 *
 ...
 * @return string
 */
function riode_responsive_callback( $settings, $value ) {
	$param_name = isset( $settings['param_name'] ) ? $settings['param_name'] : '';
	$type       = isset( $settings['type'] ) ? $settings['type'] : '';
	$form_value = $value;
	$value      = ! empty( $value ) ? json_decode( $value, true ) : array();
	$class      = 'riode-wpb-responsive-container';
...
vc_add_shortcode_param( 'riode_responsive', 'riode_responsive_callback', plugins_url( '../js/riode-responsive.js', __FILE__ ) );
