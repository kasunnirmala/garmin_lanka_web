...
<?php if ( ! WC()->cart->is_empty() ) : ?>
	...
	<ul class="scrollable mini-list woocommerce-mini-cart cart_list product_list_widget <?php echo esc_attr( $args['list_class'] ); ?>">
		<?php
			...
			if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_widget_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
				...
				<li class="woocommerce-mini-cart-item mini-item <?php echo esc_attr( apply_filters( 'woocommerce_mini_cart_item_class', 'mini_cart_item', $cart_item, $cart_item_key ) ); ?>">
					...
					if ( empty( $product_permalink ) ) :
						echo riode_escaped( $thumbnail ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					else :
					...
					<div class="cart-item-meta mini-item-meta">
						...
						echo apply_filters( 'woocommerce_widget_cart_item_quantity', '<span class="quantity"><span class="count">' . $cart_item['quantity'] . '</span>X' . $product_price . '</span>', $cart_item, $cart_item_key ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						?>
					</div>
					<?php
