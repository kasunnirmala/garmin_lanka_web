...
<head>
	...
	<?php
	if ( file_exists( wp_normalize_path( wp_upload_dir()['basedir'] . '/riode_styles/theme.min.css' ) ) ) {
		$theme_style_url = wp_upload_dir()['baseurl'] . '/riode_styles/theme.min.css?ver=' . RIODE_VERSION;
	} else {
		$theme_style_url = RIODE_CSS . '/theme/theme.min.css?ver=' . RIODE_VERSION;
	}
	?>
...
<h1>
	<?php echo riode_escaped( $localized_table_text ); ?>
	<?php
	if ( ! $is_iframe ) :
		?>
	...
	<?php else : ?>
		...
		<?php if ( 'yes' == $repeat_price && isset( $fields['price'] ) ) : ?>
			<tr class="price repeated">
				<th><?php echo riode_escaped( $fields['price'] ); ?></th>
				...
				foreach ( $products as $product_id => $product ) :
					$product_class = ( $index % 2 ? 'even' : 'odd' ) . ' product_' . $product_id
					?>
					<td class="<?php echo esc_attr( $product_class ); ?>"><?php echo riode_escaped( $product->fields['price'] ); ?></td>
					<?php
					++$index;
				endforeach;
				?>
		...
		<?php if ( 'yes' == $repeat_add_to_cart && isset( $fields['add-to-cart'] ) ) : ?>
			<tr class="add-to-cart repeated">
				<th><?php echo riode_escaped( $fields['add-to-cart'] ); ?></th>
